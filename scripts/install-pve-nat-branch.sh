...
check_debian12() {
  ...
  [[ "${VERSION_CODENAME:-}" == "bookworm" ]] || die "This script is written for Debian 12 (bookworm)."
}
...
deb [arch=amd64] ${PVE_MIRROR} bookworm pve-no-subscription
EOF
}

cleanup_old_kernel_packages() {
  info "Removing old Debian kernel packages if present"
  apt-get remove -y vlan ifupdown || true
  apt-get remove -y 'linux-image-5.10*' 'linux-headers-5.10*' || true
...
install_pve_stack() {
  ...
  DEBIAN_FRONTEND=noninteractive apt-get remove -y systemd-timesyncd os-prober || true
  update-grub || true
  dpkg --configure -a
...
write_env() {
  ...
  prompt_if_empty PVE_IMAGE_TEMPLATES 'Enter template map JSON (example: {"debian-12":9000})'
...
PVE_NAT_RETURN_IP=${NAT_RETURN_IP}
